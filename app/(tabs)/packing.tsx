...
export default function PackingScreen() {
  const { trips, currentTripId, isDarkMode, themeColors } = useTravelContext();
  ...
  return (
    <KeyboardWrapper style={[styles.container, { backgroundColor: themeColors.background }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.header, { backgroundColor: themeColors.primary }]}>
        ...
        {smartTip && (
          <View style={[styles.smartTipBox, { backgroundColor: isDarkMode ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.2)' }]}>
            <Text style={[styles.smartTipText, { color: '#FFF' }]}>🤖 {smartTip}</Text>
          </View>
        )}
      ...
      <ScrollView style={{ flex: 1, padding: 12 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        {CATEGORIES.map(cat => {
